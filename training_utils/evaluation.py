...
@ray.remote
def process_item_with_custom_reward(task_name, data_source, response_lst, reward_data):
...
def main(config):
    ...
    remote_tasks = [
        process_item_with_custom_reward.remote(task_name, data_sources[i], responses[i], reward_model_data[i]) 
